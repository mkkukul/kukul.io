  ...
  return (
    <div className="w-full max-w-2xl mx-auto space-y-6">
      <div 
        className={`relative group flex flex-col items-center justify-center w-full min-h-[300px] rounded-3xl border-2 border-dashed transition-all duration-300 ease-in-out cursor-pointer
          ${dragActive 
            ...
            : 'border-slate-300 bg-white hover:border-brand-400 hover:bg-slate-50 hover:shadow-lg'
          }
          ${isLoading ? 'pointer-events-none opacity-80 border-transparent bg-slate-50' : ''}
        `}
            ...
            {isLoading ? (
                <>
                    ...
                    <div className="w-full bg-slate-200 rounded-full h-2.5 overflow-hidden mb-2">
                        <div 
                            className="bg-brand-600 h-2.5 rounded-full transition-all duration-300 ease-out relative" 
                    ...
                    <div className="flex justify-between w-full text-xs text-slate-400 font-mono px-1">
                ...
                <>
                    <div className="mb-4 p-4 bg-brand-50 rounded-full group-hover:scale-110 transition-transform duration-300">
                        <UploadCloud className="w-10 h-10 text-brand-600" />
                    </div>
                    <div className="flex gap-2">
                        <span className="px-2 py-1 bg-slate-100 rounded text-xs text-slate-500 font-mono">JPG</span>
                        <span className="px-2 py-1 bg-slate-100 rounded text-xs text-slate-500 font-mono">PDF</span>
                        <span className="px-2 py-1 bg-slate-100 rounded text-xs text-slate-500 font-mono">Çoklu Dosya</span>
                    </div>
                </>
            )}
        </div>
      ...
      {selectedFiles.length > 0 && !isLoading && (
        <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm animate-fade-in-up">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
                {selectedFiles.map((file, index) => (
                    <div key={index} className="flex items-center gap-2 bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200 text-sm text-slate-700">
            ...
            <button 
                ...
                className="w-full py-3 bg-brand-600 hover:bg-brand-700 text-white rounded-xl font-semibold shadow-lg shadow-brand-500/30 flex items-center justify-center gap-2 transition-all transform hover:scale-[1.01]"
            >
      ...
      {!isLoading && selectedFiles.length === 0 && (
          <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
            <div className="p-4 bg-white rounded-xl shadow-sm border border-slate-100">
                ...
                <h3 className="text-sm font-semibold text-slate-800">Trend Analizi</h3>
            </div>
            <div className="p-4 bg-white rounded-xl shadow-sm border border-slate-100">
                <div className="w-8 h-8 bg-blue-100 rounded-lg flex items-center justify-center mx-auto mb-2">
                    <Loader2 className="w-4 h-4 text-blue-600" />
                </div>
                <h3 className="text-sm font-semibold text-slate-800">LGS Simülasyonu</h3>
            </div>
            <div className="p-4 bg-white rounded-xl shadow-sm border border-slate-100">
                <div className="w-8 h-8 bg-emerald-100 rounded-lg flex items-center justify-center mx-auto mb-2">
                    <FileText className="w-4 h-4 text-emerald-600" />
                </div>
                <h3 className="text-sm font-semibold text-slate-800">Nokta Atışı</h3>
            </div>
          </div>
      )}
    </div>
