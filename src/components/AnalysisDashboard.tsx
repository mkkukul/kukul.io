...
import { 
  ...
  Download, Loader2, MessageCircle, Send, Bot, Zap, PenTool, Mic, Brain, Grid3X3, FileText, Puzzle
} from 'lucide-react';
...
const METHODOLOGY_ICONS: Record<string, any> = {
    ...
    '4MAT': Brain,
    'Tic-Tac-Toe': Grid3X3,
    'Giriş Noktası': Puzzle
};

const getMethodologyIcon = (type: string) => {
    if (!type) return Lightbulb;
    const key = Object.keys(METHODOLOGY_ICONS).find(k => type.includes(k));
    return key ? METHODOLOGY_ICONS[key] : Lightbulb;
};
                             ...
                             <div key={idx} className="bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm border border-slate-200 dark:border-slate-700 h-full flex flex-col">
                                 ...
                                 <div className="space-y-4 flex-grow">
                                     {lessonPlan.sort((a,b) => (b.onem_derecesi || 0) - (a.onem_derecesi || 0)).map((item, i) => {
                                         const MethodIcon = getMethodologyIcon(item.gorev_tipi || '');
                                         
                                         return (
                                         <div key={i} className={`p-3 rounded-xl border-l-4 hover:shadow-md transition-shadow duration-200 relative overflow-hidden ${item.oncelik === 1 ? 'bg-red-50 dark:bg-red-900/10 border-red-500' : item.oncelik === 2 ? 'bg-amber-50 dark:bg-amber-900/10 border-amber-500' : 'bg-blue-50 dark:bg-blue-900/10 border-blue-500'}`}>
                                             
                                             {/* Methodology Badge */}
                                             <div className="flex items-center gap-1.5 mb-2 bg-white dark:bg-slate-800 w-fit px-2 py-1 rounded-md border border-slate-100 dark:border-slate-700 shadow-sm">
                                                 <MethodIcon className="w-3 h-3 text-brand-600 dark:text-brand-400" />
                                                 <span className="text-[10px] font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wide">
                                                     {item.gorev_tipi || 'Genel Çalışma'}
                                                 </span>
                                             </div>
